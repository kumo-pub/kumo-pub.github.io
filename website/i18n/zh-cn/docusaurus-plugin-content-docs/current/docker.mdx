...
kumo 已内置一套开发级 Docker 环境，该环境依托 kumo 入口工具 kmdo 实现底层驱动。
具体细节如下：
...
除此之外，我们亦可能推出其他 Docker 镜像版本，您可通过 [github packages](https://github.com/orgs/kumose/packages) 获取完整的 Docker 镜像列表。
