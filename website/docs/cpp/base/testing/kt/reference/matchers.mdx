...
[`EXPECT_TRUE` and `EXPECT_FALSE`](assertions.mdx#boolean) assertions.
...
[here](../advanced.mdx#regular-expression-syntax). All of these matchers, except
...
| `SafeMatcherCast<T>(m)` | [safely casts](../kmock_cook_book.mdx#SafeMatcherCast) matcher `m` to type `Matcher<T>`. |
