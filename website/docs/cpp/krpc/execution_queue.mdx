...
类似于kylin的ExecMan, [ExecutionQueue](https://github.com/apache/krpc/blob/master/src/kthread/execution_queue.h)提供了异步串行执行的功能。ExecutionQueue的相关技术最早使用在RPC中实现[多线程向同一个fd写数据](io.mdx#发消息). 在r31345之后加入到kthread。 ExecutionQueue 提供了如下基本功能:
...
- 如果临界区非常小，竞争又不是很激烈，优先选择使用mutex, 之后可以结合[contention profiler](contention_profiler.mdx)来判断mutex是否成为瓶颈。
