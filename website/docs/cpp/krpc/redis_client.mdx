...
[redis](http://redis.io/)是最近几年比较火的缓存服务，相比memcached在server端提供了更多的数据结构和操作方法，简化了用户的开发工作。为了使用户更快捷地访问redis并充分利用kthread的并发能力，krpc直接支持redis协议。示例程序：[example/redis_c++](https://github.com/apache/krpc/tree/master/example/redis_c++/)

相比使用[hiredis](https://github.com/redis/hiredis)(官方client)的优势有：

- 线程安全。用户不需要为每个线程建立独立的client。
- 支持同步、异步、批量同步、批量异步等访问方式，能使用ParallelChannel等组合访问方式。
- 支持多种[连接方式](client.mdx#连接方式)。支持超时、backup request、取消、tracing、内置服务等一系列RPC基本福利。
- 一个进程中的所有krpc client和一个redis-server只有一个连接。多个线程同时访问一个redis-server时更高效（见[性能](#性能)）。无论reply的组成多复杂，内存都会连续成块地分配，并支持短串优化(SSO)进一步提高性能。

像http一样，krpc保证在最差情况下解析redis reply的时间复杂度也是O(N)，N是reply的字节数，而不是O($N^2$)。当reply是个较大的数组时，这是比较重要的。
...
```c++
#include <krpc/redis.h>
#include <krpc/channel.h>
  
krpc::ChannelOptions options;
options.protocol = krpc::PROTOCOL_REDIS;
krpc::Channel redis_channel;
...
krpc::RedisRequest set_request;
krpc::RedisResponse response;
...
// 执行"INCR <my_key>"
krpc::RedisRequest incr_request;
...
for (int i = 0; i < 4; ++i) {
    ...
    CHECK_EQ(krpc::REDIS_REPLY_INTEGER, response.reply(i).type());
}
...
```c++
krpc::ChannelOptions options;
krpc::policy::RedisAuthenticator* auth = new krpc::policy::RedisAuthenticator("my_password");
options.auth = auth;
```

## RedisRequest

一个[RedisRequest](https://github.com/apache/krpc/blob/master/src/krpc/redis.h)可包含多个Command，调用AddCommand*增加命令，成功返回true，失败返回false**并会打印调用处的栈**。

```c++
bool AddCommand(const char* fmt, ...);
bool AddCommandV(const char* fmt, va_list args);
bool AddCommandByComponents(const kutil::StringPiece* components, size_t n);
```
...
[RedisResponse](https://github.com/apache/krpc/blob/master/src/krpc/redis.h)可能包含一个或多个[RedisReply](https://github.com/apache/krpc/blob/master/src/krpc/redis_reply.h)，reply_size()可获得reply的个数，reply(i)可获得第i个reply的引用（从0计数）。注意在hiredis中，如果请求包含了N个command，获取结果也要调用N次redisGetReply。但在krpc中这是不必要的，RedisResponse已经包含了N个reply，通过reply(i)获取就行了。只要RPC成功，response.reply_size()应与request.command_size()相等，除非redis-server有bug，redis-server工作的基本前提就是reply和command按序一一对应。
...
## 查看发出的请求和收到的回复
 ...
 打开[-redis_verbose](http://krpc.baidu.com:8765/flags/redis_verbose)即看到所有的redis request和response，注意这应该只用于线下调试，而不是线上程序。

打开[-redis_verbose_crlf2space](http://krpc.baidu.com:8765/flags/redis_verbose_crlf2space)可让打印内容中的CRLF (\r\n)变为空格，方便阅读。

| Name                     | Value | Description                              | Defined At                         |
| ------------------------ | ----- | ---------------------------------------- | ---------------------------------- |
| redis_verbose            | false | [DEBUG] Print EVERY redis request/response | src/krpc/policy/redis_protocol.cpp |
| redis_verbose_crlf2space | false | [DEBUG] Show \r\n as a space             | src/krpc/redis.cpp                 |

## 性能

redis版本：2.6.14

分别使用1，50，200个kthread同步压测同机redis-server，延时单位均为微秒。

```
$ ./client -use_kthread -thread_num 1
...
$ ./client -use_kthread -thread_num 50
...
$ ./client -use_kthread -thread_num 200
...
200个线程后qps基本到极限了。这里的极限qps比hiredis高很多，原因在于krpc默认以单链接访问redis-server，多个线程在写出时会[以wait-free的方式合并](io.mdx#发消息)，从而让redis-server就像被批量访问一样，每次都能从那个连接中读出一批请求，从而获得远高于非批量时的qps。下面通过连接池访问redis-server时qps的大幅回落是另外一个证明。

分别使用1，50，200个kthread一次发送10个同步压测同机redis-server，延时单位均为微秒。

```
$ ./client -use_kthread -thread_num 1 -batch 10  
...
$ ./client -use_kthread -thread_num 50 -batch 10
...
$ ./client -use_kthread -thread_num 200 -batch 10
...
注意redis-server实际处理的qps要乘10。乘10后也差不多在40万左右。另外在thread_num为50或200时，redis-server的CPU已打满。注意redis-server是[单线程reactor](threading_overview.mdx#单线程reactor)，一个核心打满就意味server到极限了。

使用50个kthread通过连接池方式同步压测同机redis-server。

```
$ ./client -use_kthread -connection_type pooled
...
[example/redis_c++/redis_cli](https://github.com/apache/krpc/blob/master/example/redis_c%2B%2B/redis_cli.cpp)是一个类似于官方CLI的命令行工具，以展示krpc对redis协议的处理能力。当使用krpc访问redis-server出现不符合预期的行为时，也可以使用这个CLI进行交互式的调试。
...
demostration of krpc's capability of talking to redis server. The 
...
(integer) 27
redis 127.0.0.1:6379> client setname krpc-cli
OK
