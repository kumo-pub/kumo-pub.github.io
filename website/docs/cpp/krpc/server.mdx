...
Echo的[server端代码](https://github.com/apache/krpc/blob/master/example/echo_c++/server.cpp)。
...
Service在插入[krpc.Server](https://github.com/apache/krpc/blob/master/src/krpc/server.h)后才可能提供服务。

当客户端发来请求时，Echo()会被调用。参数的含义分别是：

**controller**

在krpc中可以静态转为krpc::Controller（前提是代码运行krpc.Server中），包含了所有request和response之外的参数集合，具体接口查阅[controller.h](https://github.com/apache/krpc/blob/master/src/krpc/controller.h)
...
用户可以为http访问设置[status-code](http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html)，在server端一般是调用`controller.http_response().set_status_code()`，标准的status-code定义在[http_status_code.h](https://github.com/apache/krpc/blob/master/src/krpc/http_status_code.h)中。Controller.SetFailed也会设置status-code，值是与错误码含义最接近的status-code，没有相关的则填500错误(krpc::HTTP_STATUS_INTERNAL_SERVER_ERROR)。如果你要覆盖status_code，设置代码一定要放在SetFailed()后，而不是之前。

### 获取Client的地址

`controller->remote_side()`可获得发送该请求的client地址和端口，类型是kutil::EndPoint。如果client是nginx，remote_side()是nginx的地址。要获取真实client的地址，可以在nginx里设置`proxy_header ClientIp $remote_addr;`, 在rpc中通过`controller->http_request().GetHeader("ClientIp")`获得对应的值。

打印方式：

```c++
LOG(INFO) << "remote_side=" << cntl->remote_side(); 
printf("remote_side=%s\n", kutil::endpoint2str(cntl->remote_side()).c_str());
```

### 获取Server的地址

controller->local_side()获得server端的地址，类型是kutil::EndPoint。

打印方式：

```c++
LOG(INFO) << "local_side=" << cntl->local_side(); 
printf("local_side=%s\n", kutil::endpoint2str(cntl->local_side()).c_str());
```
...
异步service的最后一行一般是done_guard.release()以确保正常退出CallMethod时不会调用done->Run()。例子请看[example/session_data_and_thread_local](https://github.com/apache/krpc/tree/master/example/session_data_and_thread_local/)。
...
if (server.AddService(&my_echo_service, krpc::SERVER_DOESNT_OWN_SERVICE) != 0) {
...
调用以下[Server](https://github.com/apache/krpc/blob/master/src/krpc/server.h)的接口启动服务。
...
关于IPV6和Unix domain socket的使用，详见 [EndPoint](endpoint.mdx)。

`options`为NULL时所有参数取默认值，如果你要使用非默认值，这么做就行了：

```c++
krpc::ServerOptions options;  // 包含了默认值
options.xxx = yyy;
...
...
当client看到ELOGOFF时，会跳过对应的server，并在其他server上重试对应的请求。所以在一般情况下krpc总是“优雅退出”的，重启或上线时几乎不会或只会丢失很少量的流量。
...
以[echo server](https://github.com/apache/krpc/blob/master/example/echo_c%2B%2B/server.cpp)为例，你可以用[curl](https://curl.haxx.se/)访问这个服务。

```shell
# -H 'Content-Type: application/json' is optional
$ curl -d '{"message":"hello"}' http://krpc.baidu.com:8765/EchoService/Echo
{"message":"hello"}
```
...
json字段通过匹配的名字和结构与pb字段一一对应。json中一定要包含pb的required字段，否则转化会失败，对应请求会被拒绝。json中可以包含pb中没有定义的字段，但它们会被丢弃而不会存入pb的unknown字段。转化规则详见[`json <=> protobuf`](json2pb.mdx)。
...
早期的krpc允许一个pb service被http协议访问时不填充pb请求，即使里面有required字段。一般来说这种service会自行解析http请求和设置http回复，并不会访问pb请求。但这也是非常危险的行为，毕竟这是pb service，但pb请求却是未定义的。

这种服务在升级到新版本rpc时会遇到障碍，因为krpc已不允许这种行为。为了帮助这种服务升级，krpc允许经过一些设置后不把http body自动转化为pb request(从而可自行处理），方法如下：

```c++
krpc::ServiceOptions svc_opt;
svc_opt.ownership = ...;
...
如此设置后service收到http/h2请求后不会尝试把body转化为pb请求，所以pb请求总是未定义状态，用户得在`cntl->request_protocol() == krpc::PROTOCOL_HTTP || cntl->request_protocol() == krpc::PROTOCOL_H2`成立时自行解析body。
...
- [kumo标准协议](kumo_std.mdx)，显示为"baidu_std"，默认启用。

- [流式RPC协议](streaming_rpc.mdx)，显示为"streaming_rpc", 默认启用。
...
- 百盟的协议，显示为”nova_pbrpc“, 默认不启用，开启方式：
  ...
  ```c++
  #include <krpc/policy/nova_pbrpc_protocol.h>
  ...
  ServerOptions options;
  ...
  options.nshead_service = new krpc::policy::NovaServiceAdaptor;
  ```

- public_pbrpc协议，显示为"public_pbrpc"，默认不启用，开启方式：

  ```c++
  #include <krpc/policy/public_pbrpc_protocol.h>
  ...
  ServerOptions options;
  ...
  options.nshead_service = new krpc::policy::PublicPkrpcServiceAdaptor;
  ```

- nshead+mcpack协议，显示为"nshead_mcpack"，默认不启用，开启方式：

  ```c++
  #include <krpc/policy/nshead_mcpack_protocol.h>
  ...
  ServerOptions options;
  ...
  options.nshead_service = new krpc::policy::NsheadMcpackAdaptor;
  ```
...
## fork without exec
一般来说，[fork](https://linux.die.net/man/3/fork)出的子进程应尽快调用[exec](https://linux.die.net/man/3/exec)以重置所有状态，中间只应调用满足async-signal-safe的函数。这么使用fork的krpc程序在之前的版本也不会有问题。

但在一些场景中，用户想直接运行fork出的子进程，而不调用exec。由于fork只复制其调用者的线程，其余线程便随之消失了。对应到krpc中，kvar会依赖一个sampling_thread采样各种信息，在fork后便消失了，现象是很多kvar归零。

最新版本的krpc会在fork后重建这个线程(如有必要)，从而使kvar在fork后能正常工作，再次fork也可以。已知问题是fork后cpu profiler不正常。然而，这并不意味着用户可随意地fork，不管是krpc还是上层应用都会大量地创建线程，它们在fork后不会被重建，因为：
* 大部分fork会紧接exec，浪费了重建
* 给代码编写带来很多的麻烦和复杂度

krpc的策略是按需创建这类线程，同时fork without exec必须发生在所有可能创建这些线程的代码前。具体地说，至少**发生在初始化所有Server/Channel/应用代码前**，越早越好，不遵守这个约定的fork会导致程序不正常。另外，不支持fork without exec的lib相当普遍，最好避免这种用法。
...
打开[-log_idle_connection_close](http://krpc.baidu.com:8765/flags/log_idle_connection_close)后关闭前会打印一条日志。

| Name                      | Value | Description                              | Defined At          |
| ------------------------- | ----- | ---------------------------------------- | ------------------- |
| log_idle_connection_close | false | Print log when an idle connection is closed | src/krpc/socket.cpp |
...
此功能只对[kutil/logging.h](https://github.com/apache/krpc/blob/master/src/kutil/logging.h)中的日志宏有效。

打开[-log_hostname](http://krpc.baidu.com:8765/flags/log_hostname)后每条日志后都会带本机名称，如果所有的日志需要汇总到一起进行分析，这个功能可以帮助你了解某条日志来自哪台机器。

### 打印FATAL日志后退出程序

此功能只对[kutil/logging.h](https://github.com/apache/krpc/blob/master/src/kutil/logging.h)中的日志宏有效，glog默认在FATAL日志时crash。

打开[-crash_on_fatal_log](http://krpc.baidu.com:8765/flags/crash_on_fatal_log)后如果程序使用LOG(FATAL)打印了异常日志或违反了CHECK宏中的断言，那么程序会在打印日志后abort，这一般也会产生coredump文件，默认不打开。这个开关可在对程序的压力测试中打开，以确认程序没有进入过严重错误的分支。

> 一般的惯例是，ERROR表示可容忍的错误，FATAL代表不可逆转的错误。

### 最低日志级别

此功能由[kutil/logging.h](https://github.com/apache/krpc/blob/master/src/kutil/logging.h)和glog各自实现，为同名选项。
...
server的框架部分一般不针对个别client打印错误日志，因为当大量client出现错误时，可能导致server高频打印日志而严重影响性能。但有时为了调试问题，或就是需要让server打印错误，打开参数[-log_error_text](http://krpc.baidu.com:8765/flags/log_error_text)即可。

### 定制延时的分位值

显示的服务延时分位值**默认**为**80** (曾经为50), 90, 99, 99.9, 99.99，前三项可分别通过-kvar_latency_p1, -kvar_latency_p2, -kvar_latency_p3三个gflags定制。

以下是正确的设置：
```shell
-kvar_latency_p3=97   # p3从默认99修改为97
-kvar_latency_p1=60 -kvar_latency_p2=80 -kvar_latency_p3=95
```
以下是错误的设置：
```shell
-kvar_latency_p3=100   # 设置值必须在[1,99]闭区间内，gflags解析会失败
-kvar_latency_p1=-1    # 同上
```

### 设置栈大小

krpc的Server是运行在kthread之上，默认栈大小为1MB，而pthread默认栈大小为10MB，所以在pthread上正常运行的程序，在kthread上可能遇到栈不足。
...
为了保护server和client，当server收到的request或client收到的response过大时，server或client会拒收并关闭连接。此最大尺寸由[-max_body_size](http://krpc.baidu.com:8765/flags/max_body_size)控制，单位为字节。

超过最大消息时会打印如下错误日志：

```
FATAL: 05-10 14:40:05: * 0 src/krpc/input_messenger.cpp:89] A message from 127.0.0.1:35217(protocol=baidu_std) is bigger than 67108864 bytes, the connection will be closed. Set max_body_size to allow bigger messages
```
...
krpc移除了protobuf中的限制，全交由此选项控制，只要-max_body_size足够大，用户就不会看到错误日志。此功能对protobuf的版本没有要求。

### 压缩

set_response_compress_type()设置response的压缩方式，默认不压缩。

注意附件不会被压缩。HTTP body的压缩方法见[这里](http_service.mdx#压缩response-body)。

支持的压缩方法有：

- krpc::CompressTypeSnappy : [snanpy压缩](http://google.github.io/snappy/)，压缩和解压显著快于其他压缩方法，但压缩率最低。
- krpc::CompressTypeGzip : [gzip压缩](http://en.wikipedia.org/wiki/Gzip)，显著慢于snappy，但压缩率高
- krpc::CompressTypeZlib : [zlib压缩](http://en.wikipedia.org/wiki/Zlib)，比gzip快10%~20%，压缩率略好于gzip，但速度仍明显慢于snappy。

更具体的性能对比见[Client-压缩](client.mdx#压缩).
...
要开启SSL，首先确保代码依赖了最新的openssl库。如果openssl版本很旧，会有严重的安全漏洞，支持的加密算法也少，违背了开启SSL的初衷。然后设置`ServerOptions.ssl_options`，具体见[ssl_options.h](https://github.com/apache/krpc/blob/master/src/krpc/ssl_options.h)。
...
- SSL开启后，端口仍然支持非SSL的连接访问，Server会自动判断哪些是SSL，哪些不是。如果要屏蔽非SSL访问，用户可通过`Controller::is_ssl()`判断是否是SSL，同时在[connections](connections.mdx)内置监控上也可以看到连接的SSL信息。
...
你不能认为Server就用了这么多线程，因为进程内的所有Server和Channel会共享线程资源，线程总数是所有ServerOptions.num_threads和-kthread_concurrency中的最大值。比如一个程序内有两个Server，num_threads分别为24和36，kthread_concurrency为16。那么worker线程数为max(24, 36, 16) = 36。这不同于其他RPC实现中往往是加起来。

Channel没有相应的选项，但可以通过选项-kthread_concurrency调整。

另外，krpc**不区分IO线程和处理线程**。krpc知道如何编排IO和处理代码，以获得更高的并发度和线程利用率。

### 限制最大并发

“并发”可能有两种含义，一种是连接数，一种是同时在处理的请求数。这里提到的是后者。

在传统的同步server中，最大并发不会超过工作线程数，设定工作线程数量一般也限制了并发。但krpc的请求运行于kthread中，M个kthread会映射至N个worker中（一般M大于N），所以同步server的并发度可能超过worker数量。另一方面，虽然异步server的并发不受线程数控制，但有时也需要根据其他因素控制并发量。

krpc支持设置server级和method级的最大并发，当server或method同时处理的请求数超过并发度限制时，它会立刻给client回复**krpc::ELIMIT**错误，而不会调用服务回调。看到ELIMIT错误的client应重试另一个server。这个选项可以防止server出现过度排队，或用于限制server占用的资源。
...
`server.MaxConcurrencyOf("...") = ...`可设置method级别的max_concurrency。也可以通过设置`ServerOptions.method_max_concurrency`一次性为所有的method设置最大并发。
当`ServerOptions.method_max_concurrency和server.MaxConcurrencyOf("...")=...`同时被设置时，使用server.MaxConcurrencyOf()所设置的值。
...
```
关于自适应限流的更多细节可以看[这里](auto_concurrency_limiter.mdx)

### pthread模式

用户代码（客户端的done，服务器端的CallMethod）默认在栈为1MB的kthread中运行。但有些用户代码无法在kthread中运行，比如：

- JNI会检查stack layout而无法在kthread中运行。
...
对于这些情况，krpc提供了pthread模式，开启**-usercode_in_pthread**后，用户代码均会在pthread中运行，原先阻塞kthread的函数转而阻塞pthread。

注意：开启-usercode_in_pthread后，krpc::thread_local_data()不保证能获取到值。

打开pthread模式后在性能上的注意点：

- 同步RPC都会阻塞worker pthread，server端一般需要设置更多的工作线程(ServerOptions.num_threads)，调度效率会略微降低。
- 运行用户代码的仍然是kthread，只是很特殊，会直接使用pthread worker的栈。这些特殊kthread的调度方式和其他kthread是一致的，这方面性能差异很小。
- kthread支持一个独特的功能：把当前使用的pthread worker 让给另一个新创建的kthread运行，以消除一次上下文切换。krpc client利用了这点，从而使一次RPC过程中3次上下文切换变为了2次。在高QPS系统中，消除上下文切换可以明显改善性能和延时分布。但pthread模式不具备这个能力，在高QPS系统中性能会有一定下降。
- pthread模式中线程资源是硬限，一旦线程被打满，请求就会迅速拥塞而造成大量超时。一个常见的例子是：下游服务大量超时后，上游服务可能由于线程大都在等待下游也被打满从而影响性能。开启pthread模式后请考虑设置ServerOptions.max_concurrency以控制server的最大并发。而在kthread模式中kthread个数是软限，对此类问题的反应会更加平滑。

pthread模式可以让一些老代码快速尝试krpc，但我们仍然建议逐渐地把代码改造为使用kthread local或最好不用TLS，从而最终能关闭这个开关。
...
设置`ServerOptions.has_builtin_services = false`，可以完全禁用内置服务。

#### 转义外部可控的URL

可调用krpc::WebEscape()对url进行转义，防止恶意URI注入攻击。
...
/health页面默认返回"OK"，若需定制/health页面的内容：先继承[HealthReporter](https://github.com/apache/krpc/blob/master/src/krpc/health_reporter.h)，在其中实现生成页面的逻辑（就像实现其他http service那样），然后把实例赋给ServerOptions.health_reporter，这个实例不被server拥有，必须保证在server运行期间有效。用户在定制逻辑中可以根据业务的运行状态返回更多样的状态信息。

### 线程私有变量

百度内的检索程序大量地使用了[thread-local storage](https://en.wikipedia.org/wiki/Thread-local_storage) (缩写TLS)，有些是为了缓存频繁访问的对象以避免反复创建，有些则是为了在全局函数间隐式地传递状态。你应当尽量避免后者，这样的函数难以测试，不设置thread-local变量甚至无法运行。krpc中有三套机制解决和thread-local相关的问题。
...
session_local_data_factory的类型为[DataFactory](https://github.com/apache/krpc/blob/master/src/krpc/data_factory.h)，你需要实现其中的CreateData和DestroyData。

注意：CreateData和DestroyData会被多个线程同时调用，必须线程安全。

```c++
class MySessionLocalDataFactory : public krpc::DataFactory {
public:
...
server-thread-local与一次service回调绑定，从进service回调开始，到出service回调结束。所有的server-thread-local data会被尽量重用，在server停止前不会被删除。在实现上server-thread-local是一个特殊的kthread-local。

设置ServerOptions.thread_local_data_factory后访问krpc::thread_local_data()即可获得thread-local数据。若没有设置，krpc::thread_local_data()总是返回NULL。
...
public:
    ...
    void Echo(google::protobuf::RpcController* cntl_base,
        ...
        MyThreadLocalData* tls = static_cast<MyThreadLocalData*>(krpc::thread_local_data());
        if (tls == NULL) {
...
struct ServerOptions {
    ...
    // in server.
    // If this field is NULL, krpc::thread_local_data() is always NULL.
    // NOT owned by Server and must be valid when Server is running.
    // Default: NULL
    const DataFactory* thread_local_data_factory;
 
    // Prepare so many thread-local data before server starts, so that calls
    // to krpc::thread_local_data() get data directly rather than calling
...
thread_local_data_factory的类型为[DataFactory](https://github.com/apache/krpc/blob/master/src/krpc/data_factory.h)，你需要实现其中的CreateData和DestroyData。

注意：CreateData和DestroyData会被多个线程同时调用，必须线程安全。

```c++
class MyThreadLocalDataFactory : public krpc::DataFactory {
public:
    void* CreateData() const {
...
Session-local和server-thread-local对大部分server已经够用。不过在一些情况下，我们需要更通用的thread-local方案。在这种情况下，你可以使用kthread_key_create, kthread_key_destroy, kthread_getspecific, kthread_setspecific等函数，它们的用法类似[pthread中的函数](http://linux.die.net/man/3/pthread_key_create)。

这些函数同时支持kthread和pthread，当它们在kthread中被调用时，获得的是kthread私有变量; 当它们在pthread中被调用时，获得的是pthread私有变量。但注意，这里的“pthread私有变量”不是通过pthread_key_create创建的，使用pthread_key_create创建的pthread-local是无法被kthread_getspecific访问到的，这是两个独立的体系。由gcc的__thread，c++11的thread_local等声明的私有变量也无法被kthread_getspecific访问到。

由于krpc会为每个请求建立一个kthread，server中的kthread-local行为特殊：一个server创建的kthread在退出时并不删除kthread-local，而是还回server的一个pool中，以被其他kthread复用。这可以避免kthread-local随着kthread的创建和退出而不停地构造和析构。这对于用户是透明的。
...
// Returns 0 on success, error code otherwise.
extern int kthread_key_create(kthread_key_t* key, void (*destructor)(void* data));
...
用kthread_key_create创建一个kthread_key_t，它代表一种kthread私有变量。

用kthread_[get|set]specific查询和设置kthread私有变量。一个线程中第一次访问某个私有变量返回NULL。

在所有线程都不使用和某个kthread_key_t相关的私有变量后再删除它。如果删除了一个仍在被使用的kthread_key_t，相关的私有变量就泄露了。
...
kthread_key_t tls_key;
 
if (kthread_key_create(&tls_key, my_data_destructor) != 0) {
...
```c++
// in some thread ...
MyThreadLocalData* tls = static_cast<MyThreadLocalData*>(kthread_getspecific(tls_key));
if (tls == NULL) {  // First call to kthread_getspecific (and before any kthread_setspecific) returns NULL
    tls = new MyThreadLocalData;   // Create thread-local data on demand.
    CHECK_EQ(0, kthread_setspecific(tls_key, tls));  // set the data so that next time kthread_getspecific in the thread returns the data.
}
```
**示例代码**
...
public:
    EchoServiceImpl() {
        CHECK_EQ(0, kthread_key_create(&_tls2_key, my_thread_local_data_deleter));
    }
    ~EchoServiceImpl() {
        CHECK_EQ(0, kthread_key_delete(_tls2_key));
    };
    ...
private:
    kthread_key_t _tls2_key;
}
...
public:
    ...
    void Echo(google::protobuf::RpcController* cntl_base,
              ...
              google::protobuf::Closure* done) {
        ...
        // You can create kthread-local data for your own.
        // The interfaces are similar with pthread equivalence:
        //   pthread_key_create  -> kthread_key_create
        //   pthread_key_delete  -> kthread_key_delete
        //   pthread_getspecific -> kthread_getspecific
        //   pthread_setspecific -> kthread_setspecific
        MyThreadLocalData* tls2 = static_cast<MyThreadLocalData*>(kthread_getspecific(_tls2_key));
        if (tls2 == NULL) {
            tls2 = new MyThreadLocalData;
            CHECK_EQ(0, kthread_setspecific(_tls2_key, tls2));
        }
        ...
```
...
默认关闭，把参数-log_connection_close设置为true就打开了（支持[动态修改](flags.mdx#change-gflag-on-the-fly)）。

#### Q: 为什么server端线程数设了没用

krpc同一个进程中所有的server[共用线程](#worker线程数)，如果创建了多个server，最终的工作线程数多半是最大的那个ServerOptions.num_threads。

#### Q: 为什么client端的延时远大于server端的延时

可能是server端的工作线程不够用了，出现了排队现象。排查方法请查看[高效率排查服务卡顿](server_debugging.mdx)。

#### Q: Fail to open /proc/self/io

有些内核没这个文件，不影响服务正确性，但如下几个kvar会无法更新：
```
...
import server_side from '@site/static/img/krpc/server_side.png';
