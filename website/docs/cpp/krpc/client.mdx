...
和大部分的RPC框架不同，krpc中并没有独立的Client线程池。所有Channel和Server通过[kthread](kthread.mdx)共享相同的线程池. 如果你的程序同样使用了krpc的server, 仅仅需要设置Server的线程数。 或者可以通过[gflags](flags.mdx)设置[-kthread_concurrency](http://krpc.baidu.com:8765/flags/kthread_concurrency)来设置全局的线程数.
...
1. 创建一个[kthread_id](https://github.com/apache/krpc/blob/master/src/kthread/id.h)作为本次RPC的correlation_id。
