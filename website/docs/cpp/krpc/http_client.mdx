...
[example/http_c++](https://github.com/apache/krpc/blob/master/example/http_c++/http_client.cpp)
...
krpc::Channel可访问http/h2服务，ChannelOptions.protocol须指定为PROTOCOL_HTTP或PROTOCOL_H2。
...
options.protocol = krpc::PROTOCOL_HTTP;  // or krpc::PROTOCOL_H2
...
`cntl.response_attachment()`是回复的body，类型也是kutil::IOBuf。IOBuf可通过to_string()转化为std::string，但是需要分配内存并拷贝所有内容，如果关注性能，处理过程应直接支持IOBuf，而不要求连续内存。

## POST

默认的HTTP Method为GET，可设置为POST或[更多http method](https://github.com/apache/krpc/blob/master/src/krpc/http_method.h)。待POST的数据应置入request_attachment()，它([kutil::IOBuf](https://github.com/apache/krpc/blob/master/src/kutil/iobuf.h))可以直接append std::string或char*。
...
需要大量打印过程的body建议使用kutil::IOBufBuilder，它的用法和std::ostringstream是一样的。对于有大量对象要打印的场景，IOBufBuilder简化了代码，效率也可能比c-style printf更高。
...
krpc的http行为默认是http/1.1。

http/1.0相比http/1.1缺少长连接功能，krpc client与一些古老的http server通信时可能需要按如下方法设置为1.0。
```c++
...
krpc server会自动识别HTTP版本，并相应回复，无需用户设置。
...
若用户没有填且URL中包含host，比如 `http://www.foo.com/path` ，则 `http request` 中会包含 `Host: www.foo.com`。
...
访问body
```c++
kutil::IOBuf& buf = cntl->request_attachment();
...
- 根据[rfc2616](http://www.w3.org/Protocols/rfc2616/rfc2616-sec4.html#sec4.2)，http header的field_name不区分大小写。krpc支持大小写不敏感，同时会在打印时保持用户传入的大小写。
...
如果Server也是krpc框架实现的服务，client端希望在http/h2失败时获取krpc Server返回的真实`ErrorCode`，而不是统一设置的`EHTTP`，则需要设置GFlag`-use_http_error_code=true`。

## 压缩request body

调用Controller::set_request_compress_type(krpc::COMPRESS_TYPE_GZIP)将尝试用gzip压缩http body。
...
出于通用性考虑krpc不会自动解压response body，解压代码并不复杂，用户可以自己做，方法如下：
...
if (encoding != NULL && *encoding == "gzip") {
    ...
    if (!krpc::policy::GzipDecompress(cntl->response_attachment(), &uncompressed)) {
...
krpc client支持在读取完body前就结束RPC，让用户在RPC结束后再读取持续增长的body。注意这个功能不等同于“支持http chunked mode”，krpc的http实现一直支持解析chunked mode，这里的问题是如何让用户处理超长或无限长的body，和body是否以chunked mode传输无关。
...
1. 首先实现ProgressiveReader，接口如下：
   ...
   ```c++
   #include <krpc/progressive_reader.h>
   ...
   ...
   public:
       ...
       virtual kutil::Status OnReadOnePart(const void* data, size_t length) = 0;
...
## 发送https请求
https是http over SSL的简称，SSL并不是http特有的，而是对所有协议都有效。开启客户端SSL的一般性方法见[这里](client.mdx#开启ssl)。为方便使用，krpc会对https开头的uri自动开启SSL。
