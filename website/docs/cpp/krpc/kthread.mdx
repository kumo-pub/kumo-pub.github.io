...
是krpc使用的M:N线程库，目的是在提高程序的并发度的同时，降低编码难度，并在核数日益增多的CPU上提供更好的scalability和cache locality。”M:N“是指M个kthread会映射至N个pthread，一般M远大于N。由于linux当下的pthread实现([NPTL](http://en.wikipedia.org/wiki/Native_POSIX_Thread_Library))是1:1的，M个kthread也相当于映射至N个[LWP](http://en.wikipedia.org/wiki/Light-weight_process)。kthread的前身是Distributed Process(DP)中的fiber，一个N:1的合作式线程库，等价于event-loop库，但写的是同步代码。
...
kthread是一个M:N线程库，一个kthread被卡住不会影响其他kthread。关键技术两点：work stealing调度和butex，前者让kthread更快地被调度到更多的核心上，后者让kthread和pthread可以相互等待和唤醒。这两点协程都不需要。更多线程的知识查看[这里](threading_overview.mdx)。

##### Q: 我应该在程序中多使用kthread吗？

不应该。除非你需要在一次RPC过程中[让一些代码并发运行](kthread_or_not.mdx)，你不应该直接调用kthread函数，把这些留给krpc做更好。

##### Q：kthread和pthread worker如何对应？

pthread worker在任何时间只会运行一个kthread，当前kthread挂起时，pthread worker先尝试从本地runqueue弹出一个待运行的kthread，若没有，则随机偷另一个worker的待运行kthread，仍然没有才睡眠并会在有新的待运行kthread时被唤醒。

##### Q：kthread中能调用阻塞的pthread或系统函数吗？

可以，只阻塞当前pthread worker。其他pthread worker不受影响。

##### Q：一个kthread阻塞会影响其他kthread吗？

不影响。若kthread因kthread API而阻塞，它会把当前pthread worker让给其他kthread。若kthread因pthread API或系统函数而阻塞，当前pthread worker上待运行的kthread会被其他空闲的pthread worker偷过去运行。

##### Q：pthread中可以调用kthread API吗？

可以。kthread API在kthread中被调用时影响的是当前kthread，在pthread中被调用时影响的是当前pthread。使用kthread API的代码可以直接运行在pthread中。

##### Q：若有大量的kthread调用了阻塞的pthread或系统函数，会影响RPC运行么？

会。比如有8个pthread worker，当有8个kthread都调用了系统usleep()后，处理网络收发的RPC代码就暂时无法运行了。只要阻塞时间不太长, 这一般**没什么影响**, 毕竟worker都用完了, 除了排队也没有什么好方法.
在krpc中用户可以选择调大worker数来缓解问题, 在server端可设置[ServerOptions.num_threads](server.mdx#worker线程数)或[-kthread_concurrency](http://krpc.baidu.com:8765/flags/kthread_concurrency), 在client端可设置[-kthread_concurrency](http://krpc.baidu.com:8765/flags/kthread_concurrency).
...
- 一个实际的解决方法是[限制最大并发](server.mdx#限制最大并发), 只要同时被处理的请求数低于worker数, 自然可以规避掉"所有worker被阻塞"的情况.
- 另一个解决方法当被阻塞的worker超过阈值时(比如8个中的6个), 就不在原地调用用户代码了, 而是扔到一个独立的线程池中运行. 这样即使用户代码全部阻塞, 也总能保留几个worker处理rpc的收发. 不过目前kthread模式并没有这个机制, 但类似的机制在[打开pthread模式](server.mdx#pthread模式)时已经被实现了. 那像上面提到的, 这个机制是不是在用户代码都阻塞时也在做"无用功"呢? 可能是的. 但这个机制更多是为了规避在一些极端情况下的死锁, 比如所有的用户代码都lock在一个pthread mutex上, 并且这个mutex需要在某个RPC回调中unlock, 如果所有的worker都被阻塞, 那么就没有线程来处理RPC回调了, 整个程序就死锁了. 虽然绝大部分的RPC实现都有这个潜在问题, 但实际出现频率似乎很低, 只要养成不在锁内做RPC的好习惯, 这是完全可以规避的.

##### Q：kthread会有[Channel](https://gobyexample.com/channels)吗？
...
我们需要的往往是buffered channel，扮演的是队列和有序执行的作用，kthread提供了[ExecutionQueue](execution_queue.mdx)，可以完成这个目的。
