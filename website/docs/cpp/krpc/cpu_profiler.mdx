...
2. 定义宏KRPC_ENABLE_CPU_PROFILER, 一般加入编译参数-DKRPC_ENABLE_CPU_PROFILER。注意：KRPC_ENABLE_CPU_PROFILER宏需要定义在引用到krpc头文件(channel.h或server.h)的代码里。比如A模块引用B模块，B模块在实现中引用krpc头文件，必须在B模块的编译参数加上KRPC_ENABLE_CPU_PROFILER宏，在A模块加是没用的。
...
import echo_cpu_profiling from '@site/static/img/krpc/echo_cpu_profiling.png';

<img src={echo_cpu_profiling}/>

在Linux下，你也可以使用[pprof](https://github.com/apache/krpc/blob/master/tools/pprof)或gperftools中的pprof进行profiling。
