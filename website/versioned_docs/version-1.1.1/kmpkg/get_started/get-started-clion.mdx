...
教程指导用户完成通过 `CMake` 和 `clion` 安装和使用包的过程。

本教程向您展示如何创建一个使用 C++“Hello World”程序包含 CMake、kmpkg 和 `clion` 的“fmt”库。你将安装
...
[clion](https://www.jetbrains.com/clion/)是商业软件，需要购买使用，官方默认提供30天的试用时间。
从官方网站下载试用版进行学习和测试。
...
import clion_cmake from '@site/static/img/kmpkg/get_started/clion-cmake.png';
...
:::warning
kumo开发的c++服务是后台服务，并且是基于linux系列运行的服务端服务，对 `clion` 使用仅限于开发过程中的代码提示
和其他便利功能，服务的调试，还是基于 `命令行` 的方式为主。***强烈建议在unix系（如ubuntu，macos）作为基础开发环境，系统API
与实际生产环境相同，提升开发效率***
:::
