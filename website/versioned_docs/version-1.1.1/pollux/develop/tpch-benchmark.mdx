...
Benchmarking in Pollux is made easy with the optionally built TpchBenchmark
(pollux_tpch_benchmark) executable. To build the benchmark executable
(*_build/release/pollux/benchamrks/tpch/pollux_tpch_benchmark*), use the
...
## Use Cases for Pollux


Pollux is a library so there are multiple ways to use it. This document will
...
it turns out, is the use case used by the *Pollux TpchBenchmark Tool* below.
...
this means that the **drivers** are outside of Pollux and Gluten and is defined
...
Pollux exposes other options used for tuning that are of interest:
...
*NOTE: There is a SSD Caching option in Pollux but it to is ONLY useful in
...
If a use of Pollux matches the use case of the TcphBenchmark then it is a good
...
.. code:: shell
   ...
   $ ./_build/release/pollux/benchmarks/tpch/pollux_tpch_benchmark -helpon=TpchBenchmark
