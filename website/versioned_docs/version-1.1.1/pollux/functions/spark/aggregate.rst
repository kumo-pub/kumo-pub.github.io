...
.. spark:function:: bloom_filter_agg(hash, estimatedNumItems, numBits) -> varbinary
    ...
    In Pollux implementation, ``estimatedNumItems`` is not used.
    ...
    In Pollux, the value of ``numBits`` is automatically capped at the value of spark.bloom_filter.max_num_bits configuration property.
...
.. spark:function:: bloom_filter_agg(hash, estimatedNumItems) -> varbinary
    ...
    But Spark allows for changing the defaults while Pollux does not.
