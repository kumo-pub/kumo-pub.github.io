...
a format string compatible with JodaTime’s [DateTimeFormat](http://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html)
...
Pollux has full support for time zone rules, which are needed to perform date/time
...
list of supported timezones follow the definition [here](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)
