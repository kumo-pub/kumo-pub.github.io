...
除了 :func:`count`、:func:`count_if`、:func:`max_by`、:func:`min_by` 和
:func:`approx_distinct` 之外，所有这些聚合函数都会忽略空值，
并且在没有输入行或所有值都为空时返回空值。例如，
:func:`sum` 返回空值而不是零，而 :func:`avg` 的计数不包含空值。``coalesce`` 函数可用于将空值转换为
零。

某些聚合函数（例如 :func:`array_agg`）会根据输入值的顺序产生不同的结果。
