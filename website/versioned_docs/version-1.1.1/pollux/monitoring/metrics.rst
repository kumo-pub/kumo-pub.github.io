...
Runtime metrics are used to collect the metrics of important pollux runtime events
for monitoring purpose. The collected metrics can provide insights into the
continuous availability and performance analysis of a Pollux runtime system. For
instance, the collected data can help automatically generate alerts at an
outage. Pollux provides a framework to collect the metrics which consists of
three steps:
...
Register metrics during startup using registerPolluxMetrics() API.
...
Pollux supports five metric types:
...
.. list-table::
   ...
   * - arbitrator_global_arbitration_count
     ...
     - The number of arbitration which ensures the total allocated query capacity won't exceed
       ...
       initiate the memory arbitration request. This indicates the pollux runtime doesn't have
