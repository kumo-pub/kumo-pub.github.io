...
This page lists the facilities provided by KumoTest for creating and working
with mock objects. To use them, include the header
`kmock/kmock.h`.

## Macros {#macros}

KumoTest defines the following macros for working with mocks.
...
Creates an [expectation](../kmock_for_dummies.mdx#setting-expectations) that the
...
[matchers](../kmock_for_dummies.mdx#matchers-what-arguments-do-we-expect) that
...
each argument's matcher were a [wildcard matcher (`_`)](matchers.mdx#wildcard).
See the [Matchers Reference](matchers.mdx) for a list of all built-in matchers.
...
KumoTest passes all of the arguments as one tuple into the matcher. The
...
KumoTest provides some built-in matchers for 2-tuples, including the `Lt()`
matcher above. See [Multi-argument Matchers](matchers.mdx#MultiArgMatchers).
...
If the `Times` clause is omitted, KumoTest infers the cardinality as follows:
...
[action](../kmock_for_dummies.mdx#actions-what-should-it-do) that the function
call will perform. See the [Actions Reference](actions.mdx) for a list of
built-in actions.
...
KumoTest defines the following classes for working with mocks.
...
[uninteresting calls](../kmock_cook_book.mdx#uninteresting-vs-unexpected). The
