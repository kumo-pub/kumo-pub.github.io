...
The Alkaid Datasets library provides functionality to efficiently work with
...
The supported file formats currently are Parquet, Feather / Alkaid IPC, CSV and
...
A :class:`alkaid::dataset::Dataset` object can be created using the various
:class:`alkaid::dataset::DatasetFactory` objects. Here, we'll use the
:class:`alkaid::dataset::FileSystemDatasetFactory`, which can create a dataset
...
Creating a :class:`alkaid::dataset::Dataset` does not begin reading the data
itself. It only crawls the directory to find all the files (if needed), which can
be retrieved with :func:`alkaid::dataset::FileSystemDataset::files`:
...
Using the :func:`alkaid::dataset::Dataset::NewScan` method, we can build a
:class:`alkaid::dataset::Scanner` and read the dataset (or a portion of it) into
a :class:`alkaid::Table` with the :func:`alkaid::dataset::Scanner::ToTable`
method:
...
Currently, Parquet, ORC, Feather / Alkaid IPC, and CSV file formats are
...
…then we can read the Feather file by passing an :class:`alkaid::dataset::IpcFileFormat`:
...
:class:`alkaid::dataset::FileFormat` objects have properties that control how
...
setting :member:`alkaid::dataset::CsvFileFormat::parse_options` lets us change
things like reading comma-separated or tab-separated data.

Additionally, passing an :class:`alkaid::dataset::FragmentScanOptions` to
:func:`alkaid::dataset::ScannerBuilder::FragmentScanOptions` offers fine-grained
...
:class:`alkaid::dataset::Scanner` offers control over what data to read.

In this snippet, we use :func:`alkaid::dataset::ScannerBuilder::Project` to select
...
A filter can be provided with :func:`alkaid::dataset::ScannerBuilder::Filter`, so
...
In addition to selecting columns, :func:`alkaid::dataset::ScannerBuilder::Project`
...
.. note:: When combining filters and projections, Alkaid will determine all
          necessary columns to read. For instance, if you filter on a column that
          isn't ultimately selected, Alkaid will still read the column to evaluate
          the filter.
...
Let's create a small partitioned dataset. For this, we'll use Alkaid's dataset
...
.. code-block:: cpp
    ...
    auto part = std::make_shared<ds::HivePartitioning>(alkaid::schema({
        alkaid::field("year", alkaid::int16()),
        alkaid::field("month", alkaid::int8()),
        alkaid::field("day", alkaid::int32())
    }));

Alkaid supports another partitioning scheme, "directory partitioning", where the
...
systems will be reading the data. Most systems, including Alkaid, should work across a
...
of file size. Alkaid's file writer provides sensible defaults for group sizing in most cases.
...
in an :class:`alkaid::dataset::InMemoryDataset`:

.. code-block:: cpp

   auto table = alkaid::Table::FromRecordBatches(...);
   auto dataset = std::make_shared<alkaid::dataset::InMemoryDataset>(std::move(table));
...
In addition to local files, Alkaid Datasets also support reading from cloud
