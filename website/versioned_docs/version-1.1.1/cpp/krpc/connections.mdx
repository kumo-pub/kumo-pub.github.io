...
[connections服务](http://krpc.baidu.com:8765/connections)可以查看所有的连接。一个典型的页面如下：
...
- 第二段是server与下游的单连接（使用krpc::Channel建立），fd为-1的是虚拟连接，对应第三段中所有相同RemoteSide的连接。
...
import single_conn from '@site/static/img/krpc/single_conn.png';

单连接：

<img src={single_conn}/>

import pooled_conn from '@site/static/img/krpc/pooled_conn.png';

连接池：

<img src={pooled_conn}/>

import short_conn from '@site/static/img/krpc/short_conn.png';
