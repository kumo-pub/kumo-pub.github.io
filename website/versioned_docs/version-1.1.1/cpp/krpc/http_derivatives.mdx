...
http/h2协议的基本用法见[http_client](http_client.mdx)和[http_service](http_service.mdx)
...
[gRPC](https://github.com/grpc)的默认协议，具体格式可阅读[gRPC over HTTP2](https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-HTTP2.mdx)

使用krpc的客户端把ChannelOptions.protocol设置为"h2:grpc"一般就能连通gRPC。

使用krpc的服务端一般无需修改代码即可自动被gRPC客户端访问。
