...
类似于kylin的ExecMan, [ExecutionQueue](https://gitee.com/kumo-pub/kthread/blob/master/kthread/internal/execution_queue.h)提供了异步串行执行的功能。ExecutionQueue的相关技术最早使用在RPC中实现[多线程向同一个fd写数据](../krpc/io.mdx#发消息). 在r31345之后加入到kthread。 ExecutionQueue 提供了如下基本功能:
...
- ExecutionQueue运行线程为kthread, 可以随意的使用一些kthread同步原语而不用担心阻塞pthread的执行. 而在ExecMan里面得尽量避免使用较高概率会导致阻塞的同步原语.
...
- 如果临界区非常小，竞争又不是很激烈，优先选择使用mutex, 之后可以结合[contention profiler](../krpc/contention_profiler.mdx)来判断mutex是否成为瓶颈。
