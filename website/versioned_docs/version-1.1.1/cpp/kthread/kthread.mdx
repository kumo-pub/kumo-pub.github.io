...
```
然后创建kthread协程栈，返回值为协程栈顶context，函数入参分别为协程栈底，栈大小，以及这个kthread要执行的函数entry。
``` c++
            context = kthread_make_fcontext(storage.bottom, storage.stacksize, entry);
...
import kthread_make_fcontext from '@site/static/img/kthread/kthread_make_fcontext.png';

<img src={kthread_make_fcontext}/>
...
在协程切换过程中有两种情况，第一种为新协程是通过kthread_make_fcontext函数刚刚创建的栈，另一种是已经运行过的栈，这两种过程分别如下图所示：

import kthread_jump_fcontext from '@site/static/img/kthread/kthread_jump_fcontext.png';

<img src={kthread_jump_fcontext}/>
