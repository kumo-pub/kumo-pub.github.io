...
* 加入 [JSON Pointer](pointer.mdx) 功能，可更简单地访问及更改 DOM。
* 加入 [JSON Schema](schema.mdx) 功能，可在解析或生成 JSON 时进行校验。
* 加入 [放宽的 JSON 语法](dom.mdx) （注释、尾随逗号、NaN/Infinity）
* 使用 [C++11 范围 for 循环](tutorial.mdx) 去遍历 array 和 object。
