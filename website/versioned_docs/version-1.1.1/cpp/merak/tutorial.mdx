...
如 [用法一览](overview.mdx#用法一览) 中所示，可以解析一个 JSON 至 DOM，然后就可以轻松查询及修改 DOM，并最终转换回 JSON。
...
import tutorial from '@site/static/img/merak/tutorial.png';
...
import move1 from '@site/static/img/merak/move1.png';
...
import move2 from '@site/static/img/merak/move2.png';
...
import move3 from '@site/static/img/merak/move3.png';
...
1. [流](stream.mdx) 是读写 JSON 的通道。流可以是内存字符串、文件流等。用户也可以自定义流。
2. [编码](encoding.mdx) 定义在流或内存中使用的字符编码。RapidJSON 也在内部提供 Unicode 转换及校验功能。
3. [DOM](dom.mdx) 的基本功能已在本教程里介绍。还有更高级的功能，如原位（*in situ*）解析、其他解析选项及高级用法。
4. [SAX](sax.mdx) 是 RapidJSON 解析／生成功能的基础。学习使用 `Reader`/`Writer` 去实现更高性能的应用程序。也可以使用 `PrettyWriter` 去格式化 JSON。
5. [性能](performance.mdx) 展示一些我们做的及第三方的性能测试。
6. [技术内幕](internals.mdx) 讲述一些 RapidJSON 内部的设计及技术。

你也可以参考 [常见问题](faq.mdx)、API 文档、例子及单元测试。
